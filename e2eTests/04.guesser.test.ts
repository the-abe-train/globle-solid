...
describe('Tests with a fake answer', () => {
  ...
  test('all possible ways guesser will accept names', async () => {
    ...
    const msg2 = await page.$eval('p[data-testid="guess-msg"]', (el) => el.textContent);
    expect(msg2).toContain('"asdfasdfasdf" not found in database');

    // Close guess
    await page.type('[data-cy="guesser"]', 'swodon');
    await page.keyboard.press('Enter');
    const msg3 = await page.$eval('p[data-testid="guess-msg"]', (el) => el.textContent);
    expect(msg3).toContain('Did you mean Sweden?');

    // Correct guess
    await page.type('[data-cy="guesser"]', 'Turkey');
    await page.keyboard.press('Enter');
    const msg4 = await page.$eval('p[data-testid="guess-msg"]', (el) => el.textContent);
    expect(msg4).toContain('next guess');

    // Close enough guess
    await page.type('[data-cy="guesser"]', 'saudi arubia');
    await page.keyboard.press('Enter');
    const msg5 = await page.$eval('p[data-testid="guess-msg"]', (el) => el.textContent);
    expect(msg5).toContain('Saudi Arabia is warmer');

    // Already guessed
    await page.type('[data-cy="guesser"]', 'saudi arobia');
    await page.keyboard.press('Enter');
    const msg6 = await page.$eval('p[data-testid="guess-msg"]', (el) => el.textContent);
    ...
    await page.keyboard.press('Enter');
    const msg7 = await page.$eval('p[data-testid="guess-msg"]', (el) => el.textContent);
    ...
    await page.keyboard.press('Enter');
    const msg8 = await page.$eval('p[data-testid="guess-msg"]', (el) => el.textContent);
  ...
  test('toggle country list sort order', async () => {
    ...
    const firstItemText = await page.$eval('li', (element) => element.textContent);
    ...
    const newFirstItemText = await page.$eval('li', (element) => element.textContent);
  ...
  test('winning the game', async () => {
    ...
    const firstItemAfterWin = await page.$eval('li', (element) => element.textContent);
    ...
    await page.waitForFunction(() => document.body.innerText.includes('Statistics'));
    console.log('✅ Statistics section found');

    const gamesWon = await page.$eval('[data-cy="games-won"]', (el) => el.textContent);
    ...
    const todaysGuesses = await page.$eval('[data-cy="today\'s-guesses"]', (el) => el.textContent);
