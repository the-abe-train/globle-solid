...
export default function Toggle(props: Props) {
  ...
  const values = () => (props.toggleProp() ? props.values.on : props.values.off);
    ...
    >
      <p>
        ...
        <span data-i18n={values().i18n} data-testid="toggle-text">
          {t(values().i18n, values().default)}
        </span>
      </p>
      <div class="relative cursor-pointer">
        <div class="block h-8 w-14 rounded-full border-2 border-gray-500 bg-gray-100"></div>
        <div
          data-cy={`toggle-${defaultOn}-${defaultOff}`}
          classList={{
            'translate-x-full': props.toggleProp(),
          }}
          class="absolute top-1 left-1 h-6 w-6 rounded-full bg-blue-700 transition hover:bg-blue-800 dark:bg-purple-800 dark:hover:bg-purple-900 dark:focus:ring-purple-900"
        />
      </div>
    </div>
  );
